...
1. **多平台網址轉換**：當有人發送支援平台的連結時，機器人會：
   ...
   各平台專屬行為：
   - **Twitter/X**：在多個嵌入區塊中顯示推文的多張圖片，GIF 以動態嵌入訊息呈現，影片則貼出原始 `video.twimg.com` 連結並附上文字嵌入訊息
   - **Pixiv**：顯示作品預覽與作者資訊
   - **Bilibili**：提供影片/內容預覽
...
*   **變更：** Twitter/X 的 GIF 推文現在會以一般推文嵌入訊息呈現，並使用 fixupx 提供的動態 WebP 作為圖片，不再貼出 `fixupx.com`/`fixvx.com` 連結。
*   **變更：** Twitter/X 的影片推文現在會先以獨立訊息貼出原始 `video.twimg.com` 影片的 `[Preview](…)` 連結（讓 Discord 能直接播放），接著再貼出推文文字的嵌入訊息。此邏輯同時套用於自動訊息嵌入與 `/translate` 指令，影片不再依賴 fixupx/fixvx 的渲染；已移除 1.8.1 的 `og:video` 偵測與 fixvx 影片備用機制。

### 版本 1.8.1 (2026-09-21)

*   **問題修復：** Twitter/X 影片連結（不只 GIF）現在也會在 fixupx 無法正確渲染影片時（嵌入區塊空白或顯示「post unavailable」）改用 `fixvx.com`，做法是在使用 fixupx 連結前先偵測該頁面是否含有 `og:video` 標籤。此邏輯同時套用於自動訊息嵌入與 `/translate` 指令的影片回覆。

### 版本 1.8.0 (2026-09-16)

*   **新增功能：** 新增「轉換為GIF」右鍵應用程式集指令，可將訊息中的影片（附件或連結預覽的影片嵌入）轉換成 GIF 並發布到頻道。同時間僅處理一個轉檔請求，其餘請求會排隊等待；若轉出來的 GIF 檔案過大無法上傳，會直接告知使用者失敗，不會退回貼連結。
*   **問題修復：** Steam 特賣播報改為每天固定台灣時間中午 12:00 播報「目前所有特價」，取代原本只比對差異、且在程式重啟後會靜默漏播一整天的邏輯。
*   **問題修復：** `UrlConversionService` 原本每則訊息都會重新建立一次（連帶重新編譯 11 個平台服務的網址規則），改為共用單一實例。這同時修好了「刪除訊息」指令的權限判斷——過去因為每次都是全新、沒有紀錄的實例，判斷永遠失敗，只能靜默改用備援方式（多打一次 Discord API）。
*   **功能強化：** 為 Pixiv、E-Hentai、PChome 及圖片 metadata 讀取等對外請求補上逾時設定，避免對方沒有回應時連線無限期卡住。
