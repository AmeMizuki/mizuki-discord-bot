...
*   **變更：** Twitter/X 的 GIF 推文改以動態嵌入訊息呈現，不再貼連結。
*   **變更：** Twitter/X 的影片推文（含 `/translate`）改為貼出影片預覽連結並附上推文嵌入訊息，取代原本的 fixvx 備用機制。
*   **新增功能：** 帶語言代碼的 fixupx/fixvx 連結（例如 `/status/123/tw`）現在會以嵌入訊息顯示完整翻譯。
